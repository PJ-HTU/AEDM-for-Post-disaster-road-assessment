...
class PDRATester:
    ...
    def _test_one_batch(self, batch_size, vehicle_config=None):
        ...
        self.model.eval()
        with torch.no_grad():
            self.env.load_problems(batch_size, vehicle_config = self.vehicle_config, aug_factor = aug_factor, use_fixed_seed=False)
        ...
        print(f"最优路径 (batch_idx={batch_idx}, pomo_idx={pomo_idx}): 奖励 = {global_max_reward.item()}")
        print(f"路径节点序列: {selection_node_list[batch_idx][pomo_idx]}")

...
class PDRAModel(nn.Module):
    ...
    def pre_forward(self, reset_state, vehicle_config=None):
        depot_xy = reset_state.depot_xy  # shape: (batch, 1, 2)
        node_xy = reset_state.node_xy    # shape: (batch, problem, 2)
        node_demand = reset_state.node_demand  # shape: (batch, problem)
        node_xy_demand = torch.cat((node_xy, node_demand[:, :, None]), dim=2)  # shape: (batch, problem, 3)
        ...
        depot_xy_with_config = torch.cat((depot_xy, config_tensor), dim=2)  # shape: (batch, 1, 4)

        self.encoded_nodes = self.encoder(depot_xy_with_config, node_xy_demand)
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:    # First Move, depot
            ...
            vehicle_state = {'current_vehicle': state.current_vehicle, 'collected_demand': state.collected_demand}
            
            probs = self.decoder(encoded_last_node, state.load, ninf_mask=state.ninf_mask, vehicle_state=vehicle_state)
...
class PDRA_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        self.Wq_last = nn.Linear(embedding_dim + 1 + 2, head_num * qkv_dim, bias=False)  
        self.Wk = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
        self.Wv = nn.Linear(embedding_dim, head_num * qkv_dim, bias=False)
    ...
    def forward(self, encoded_last_node, load, ninf_mask, vehicle_state=None):
        # encoded_last_node.shape: (batch, pomo, embedding)
        # load.shape: (batch, pomo)
        # ninf_mask.shape: (batch, pomo, problem+1)
        # vehicle_state: dict with 'current_vehicle', 'collected_demand'

        head_num = self.model_params['head_num']
        batch_size = encoded_last_node.size(0)
        pomo_size = encoded_last_node.size(1)
        
        # vehicle_state['current_vehicle']：shape: (batch, pomo)
        # vehicle_state['collected_demand']：shape: (batch, pomo)
        current_vehicle_norm = vehicle_state['current_vehicle'].float()    
        collected_demand_norm = vehicle_state['collected_demand'].float()  
        vehicle_features = torch.stack([current_vehicle_norm, collected_demand_norm], dim=2)
        # shape: (batch, pomo, 2)

        input_cat = torch.cat((encoded_last_node, load[:, :, None], vehicle_features), dim=2)
